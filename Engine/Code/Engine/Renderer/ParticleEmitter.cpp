	...
	:m_renderer(renderer), m_position(position), m_texture(texture), m_size(size), m_lifeTime(lifeTime)
{
	...
	AddVertsForQuad3D(m_verts, BL, BR, TR, TL, Rgba8::WHITE, Rgba8::WHITE);
...
void Particle::Render(Camera* camera, BillboardType type) const
{
	UNUSED(camera);
	UNUSED(type);
	...
	m_renderer->BindTexture(0, m_texture);
	m_renderer->BindShader(nullptr);
	m_renderer->SetBlendMode(m_blendMode);
	m_renderer->SetDepthMode(m_depthMode);
	m_renderer->SetRasterizerMode(RasterizerMode::SOLID_CULL_NONE);
	m_renderer->SetModelConstants(RootSig::DEFAULT_PIPELINE, renderMatrix, m_color);
	m_renderer->DrawVertexBuffer(m_buffer, (int)m_verts.size(), sizeof(Vertex_PCU));
}

Mat44 Particle::GetModelMatrix() const
{
	Mat44 modelMat = Mat44();
	modelMat = m_orientation.GetAsMatrix_XFwd_YLeft_ZUp();
...
void Emitter::Update(float deltaSeconds)
{
	...
	if (m_spawnTimer <= 0.f)
	{
		...
		int numberOfParticles = m_rng.RollRandomIntInRange((int)m_numParticleEachSpawn.m_min, (int)m_numParticleEachSpawn.m_max);
...
{
	m_renderer->BeginCamera(*camera, RootSig::DEFAULT_PIPELINE);
	for (size_t i = 0; i < m_emitters.size(); i++)
	{
