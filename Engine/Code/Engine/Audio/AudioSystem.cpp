...
void AudioSystem::Startup()
{
	...
	//if (m_audioType == AudioType::TYPE_2D)
	//{
		result = m_fmodSystem->init(512, FMOD_INIT_NORMAL, nullptr);
		ValidateResult(result);
	//}
	//else if (m_audioType == AudioType::TYPE_3D)
	//{
...
SoundID AudioSystem::CreateOrGetSound3D(const std::string& soundFilePath)
{
	std::map< std::string, SoundID >::iterator found = m_registeredSoundIDs.find(soundFilePath);
	if (found != m_registeredSoundIDs.end())
	{
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, const Vec3& soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	...
	if (channelAssignedToSound)
	{
		...
		FMOD_VECTOR pos;
		pos.x = -soundPosition.y;
		pos.y = soundPosition.z;
		pos.z = -soundPosition.x;
		...
		channelAssignedToSound->setMode(playbackMode);
		channelAssignedToSound->set3DAttributes(&pos, &velocity);
		channelAssignedToSound->getFrequency(&frequency);
		channelAssignedToSound->setFrequency(frequency * speed);
		channelAssignedToSound->setVolume(volume);
		channelAssignedToSound->setPan(balance);
...
{
	FMOD_VECTOR pos;
	pos.x = -listenerPosition.y;
	pos.y = listenerPosition.z;
	pos.z = -listenerPosition.x;
	...
	m_fmodSystem->set3DListenerAttributes(listenerIndex, &pos, &velocity, &forward, &up);
}
