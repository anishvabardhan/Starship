...
struct Mat44
{
	...
	static Mat44 const CreateOrthoProjection(float minX, float maxX, float minY, float maxY, float minZ, float maxZ);
	static Mat44 const CreatePerspectiveProjection(float fov, float aspectRatio, float frontZ, float backZ);
